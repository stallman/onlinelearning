...
@section('content')
    <h1>{{ $sPageTitle }}</h1>
    <a href="{{route('admin.curators.create')}}" class="btn btn-success">Добавить</a>
    ...
    <table id="" class="table table-admin-in-entities">
        ...
        @foreach($arUsers as $obItem)
            <tr>
                ...
                <td>
                    <form action="{{route('admin.curators.destroy', $obItem)}}" method="POST" class="btn-group"
                          role="group" aria-label="Basic mixed styles example">
                        <a href="{{route('admin.curators.edit', $obItem)}}" class="btn btn-success">Редактировать</a>
                        @csrf
                        @method('DELETE')
